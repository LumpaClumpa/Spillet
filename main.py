...
maxCoins = math.ceil(lawnSize[0] * lawnSize[1] / 1000)
spawnCoin(maxCoins)
...
def draw_shop(surface):
    ...
    lawnupgrade.draw()
    lawnupgradePrice = font.render('Price: ' + str(maxCoins), False, (255, 255, 255))
    display.blit(lawnupgradePrice, (lawnupgrade.x + 10, lawnupgrade.y + 80))
...
while not done:
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if lawnupgrade.isOver(pygame.mouse.get_pos()) and shop_open and coinCount >= maxCoins:
                coinCount -= maxCoins
                lawn.fill((65, 152, 10))
                x = 0
                y = 0
                lawnSize = (lawnSize[0] + 50, lawnSize[1] + 50) #temp
                maxCoins = math.ceil(lawnSize[0] * lawnSize[1] / 1000)
                spawnCoin(maxCoins)
